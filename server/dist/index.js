...
function attributesToLookup(records) {
    const map = {};
...
function buildStatBlock(attrs, level) {
...
app.post('/api/combat/random', authRequired, async (req, res) => {
    try {
        const userId = req.userId;
...
app.get('/api/combat/:id', authRequired, async (req, res) => {
    try {
        const userId = req.userId;
